...
1) How many “seconds” in theory have passed after the start sequence developed in part (d)?
   Answer: 15 seconds. The loop increments from value=0 to value=15 (breaks when (value & 0xF) == 0xF),
...
2) In the generated assembly code, in which RISC-V register will the return values from
   functions get_btn and get_sw be placed?
   Answer: a0 (x10). Per the RISC-V ABI, a 32-bit int return value is delivered in register a0.
*/
